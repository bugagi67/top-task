...
var Task = /*#__PURE__*/function () {
  ...
  return Task_createClass(Task, [{
    key: "filterTask",
    value: function filterTask(state) {
      var query = state._inputTask.value.trim().toLowerCase();
      Array.from(state._allTasks.children).forEach(function (child) {
        return child.remove();
      });
      var filteredTasks = state._arrayAllTasks.filter(function (task) {
        var taskText = task.outerText.toLowerCase();
    ...
    key: "addTask",
    value: function addTask(elem, state) {
      var container = state._allTasks;
      var div = document.createElement("div");
      div.classList.add("task");
      container.insertAdjacentElement("beforeend", div);
      var spanTextTask = document.createElement("span");
      spanTextTask.innerText = elem;
      div.insertAdjacentElement("beforeend", spanTextTask);
      var buttonPinedTask = document.createElement("button");
      ...
      state._arrayAllTasks.forEach(function (task) {
        state._allTasks.appendChild(task);
      });
    }
  }, {
    key: "pinedTask",
    value: function pinedTask(elem, state) {
      var pinedElement = elem;
      elem.remove();
      pinedElement.classList.add("pinned");
      var buttonPinedTask = pinedElement.querySelector(".not_pinned");
      buttonPinedTask.innerText = "▼";
      ...
      state._arrayAllTasks = state._arrayAllTasks.filter(function (elem) {
        return elem !== pinedElement;
      });
    }
  }, {
    key: "unpinTask",
    value: function unpinTask(elem, state, uiManage) {
      var pinedElement = elem;
      elem.remove();
      this.checkPinnedTask(state, uiManage);
      pinedElement.classList.remove("pinned");
      var buttonUnPinedTask = pinedElement.querySelector(".pinned_task");
      ...
      state._arrayPinnedTasks = state._arrayPinnedTasks.filter(function (elem) {
        return elem !== pinedElement;
      });
    ...
    key: "checkPinnedTask",
    value: function checkPinnedTask(state, uiManage) {
...
var UiManager = /*#__PURE__*/function () {
  ...
  return UiManager_createClass(UiManager, [{
    ...
    key: "closeModal",
    value: function closeModal(state) {
      state._modal.style.display = "none";
    }
  }, {
    key: "deleteNoPined",
    value: function deleteNoPined(state) {
      state._noPinned.style.display = "none";
    }
  }, {
    key: "addNoPined",
    value: function addNoPined(state) {
...
state._inputTask.addEventListener("input", function () {
  task.filterTask(state);
});
state._modalButtonClose.addEventListener("click", function () {
  uiManager.closeModal(state);
});
state._modal.addEventListener("click", function (e) {
  if (e.target.classList.contains("modal")) {
    uiManager.closeModal(state);
  }
});
state._allTasks.addEventListener("click", function (e) {
  if (e.target.classList.contains("not_pinned")) {
    var taskElement = e.target.parentNode;
    task.pinedTask(taskElement, state);
    uiManager.deleteNoPined(state);
  }
});
state._pinnedTask.addEventListener("click", function (e) {
